		...
		<view class="container">
            <view class="detail-content">
				...
				<view class="detail-list-item" :style='{"padding":"0","boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0","borderColor":"rgba(193, 193, 193, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0 0 2rpx","width":"100%","borderStyle":"solid","height":"auto"}'>
					...
					<view  class="text" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(0,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"calc(100% - 200rpx)","lineHeight":"auto","fontSize":"28rpx","borderStyle":"solid"}'>{{detail.leixing}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','meishifenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','meishifenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('meishifenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussmeishifenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('meishifenlei', this.detail);
				this.$utils.msg('审核成功');
