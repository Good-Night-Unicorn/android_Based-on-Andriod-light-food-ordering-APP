<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.meishileixing" type="text" placeholder="美食类型" ></input>
			</view>
		...
		<view :style="2 != 1 ? 'display: flex;' : ''">
                                                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,1)","backgroundColor":"rgba(82, 142, 187, 1)","borderRadius":"0","borderWidth":"0","width":"160rpx","borderStyle":"solid","height":"auto"}'>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"20rpx 30rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"14rpx 0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"60rpx 0 60rpx 0","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
                                                                                                                                  <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255,0,0,0)","borderRadius":"0","borderWidth":"4rpx","width":"240rpx","borderStyle":"solid","height":"180rpx"}' mode="aspectFill" :src="product.meishitupian?baseUrl+product.meishitupian.split(',')[0]:''"></image>
                                                                                                                                                                                              <view class="list-item-body" style="flex: 1;">
            <view :style='{"padding":"20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}' class="title">{{product.meishimingcheng}}</view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('meishifenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('meishifenlei', {page:1,limit:100});
            }
		...
		methods: {
			...
			async search(){
				...
				if(this.searchForm.meishileixing){
					searchForm['meishileixing'] = '%' + this.searchForm.meishileixing + '%'
				}
                let res = {};
                if(this.userid) {
...
<style>
	...
	.tab {
		...
		background-color: rgba(82, 142, 187, 1);
	...
	.tabActive{
		...
		border-style: solid;
		border-color: rgba(255, 255, 255, 1);
		background-color: rgba(124, 194, 247, 1);
		box-shadow: 0 0 0px rgba(0,0,0,.3);
	}
	.tab:last-of-type {
		border: none;
